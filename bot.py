...
def miniapp_btn(label="\U0001f6d2 Katalogni ochish (Mini App)"):
            """WebApp tugmasini yaratish - telebot versiyasidan qat'iy nazar ishlaydi"""
            btn = types.InlineKeyboardButton(text=label, url=None)
            btn.web_app = types.WebAppInfo(url=MINIAPP_URL)
            return btn
...
def start(m):
            text = (
                            "\U0001f44b XASANBOY OPTOM ga xush kelibsiz!\n\n"
                            "Biz ulgurji savdo bilan shug'ullanamiz.\n\n"
                            "\U0001f447 Quyidagi tugmani bosing va katalogni oching:"
            )
            kb_inline = types.InlineKeyboardMarkup()
            try:
                            kb_inline.add(miniapp_btn())
except Exception:
        kb_inline.add(types.InlineKeyboardButton(
                            text="\U0001f310 Saytni ochish",
                            url=MINIAPP_URL
        ))
    bot.send_message(m.chat.id, text, reply_markup=main_kb())
    bot.send_message(m.chat.id, "\U0001f6d2 Mini App:", reply_markup=kb_inline)
...
def katalog(m):
            kb_inline = types.InlineKeyboardMarkup()
            try:
                            kb_inline.add(miniapp_btn("\U0001f4f1 Mini App orqali ochish"))
except Exception:
        kb_inline.add(types.InlineKeyboardButton(
                            text="\U0001f310 Saytni ochish",
                                url=MINIAPP_URL
    ...
    for cat in cats:
        kb.add(types.InlineKeyboardButton(cat, callback_data="c_" + cat))
                bot.send_message(m.chat.id, "\U0001f4f1 Mini App:", reply_markup=kb_inline)
    ...
    try:
        ...
        items = data.get("items", [])
                    total = data.get("total", 0)
                                      lines = [
            f"{it.get('name','?')} x{it.get('qty',1)} = {it.get('subtotal',0):,} so'm"
                                        for it in items
]
                    admin_text = (
